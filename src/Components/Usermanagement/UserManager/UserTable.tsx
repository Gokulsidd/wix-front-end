...
export default function UserManagerTable({
  ...
  const deleteallusers = async () => {
    setBtnLoading(true);
    await UserMgtApi.deleteallusers(selected)
      .then((res: any) => {
  ...
  const deleteuser = async () => {
    setBtnLoading(true);
    await UserMgtApi.deleteuser(delId)
      .then((res: any) => {
        toast.success(res.message);
        setDelId(null);
        getUsers();
    ...
    return (
      <Dropdown>
        ...
        <Menu size="sm" sx={{ minWidth: 140 }}>
          ...
          <MenuItem
            onClick={() => {
              navigate("/dashboard/add_user_admin", { state: props.props });
            }}
          >
            <EditIcon />
            Edit
  ...
  return (
    <Box>
      <ConfirmDelete
        ...
        deleteEvent={confirm}
      />
      <UserView open={viewOpen} setOpen={setViewOpen} />
      <Sheet
      ...
      >
        ...
        >
          ...
          <tbody>
            {stableSort(rows, getComparator(order, "name")).map(
              (row: any, index: number) => (
                <tr key={index}>
                  ...
                  <td>
                    <Typography level="body-xs">{row.emp_id}</Typography>
                  </td>
                  <td>
